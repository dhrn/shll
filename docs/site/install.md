...
When Homebrew is absent the script **bootstraps it headlessly** — the official installer with `NONINTERACTIVE=1` (on macOS this also installs the Command Line Tools via `softwareupdate`; on Linux the preflight has already guaranteed git). A fresh Homebrew isn't on `PATH` in the current shell, so the script evals `brew shellenv` itself before handing off, and persists it for your future shells: it writes the line into the rc file `shll setup shell` wires (`${ZDOTDIR:-$HOME}/.zshrc` for zsh; `~/.bashrc` for bash on Linux, `~/.bash_profile` on macOS), above the shll block, since that block calls `shll` and needs brew on `PATH` first:
...
An rc file that already mentions `brew shellenv` is left alone. For any other shell, or an rc file it can't write, the script prints the line for you to add instead.
